...
export default function DevicePrintCursor() {
  ...
  useEffect(() => {
    ...
    fetch(
      `${SERVER_ADDRESS}/api/print/device_status_cursor?${params.toString()}`
    )
      .then((r) => r.json())
      .then((json) => {
        if (!json?.success) {
      ...
      .catch((e) => {
        console.error("❌ device_status_cursor error:", e);
        setPageData([]);
        setHasNext(false);
        setNextCursor(null);
      })
    ...
    const run = async () => {
      ...
      try {
        const warmRes = await fetch(
          `${SERVER_ADDRESS}/api/print/device_status_cursor?generator_name=${encodeURIComponent(
            generatorName
          )}&limit=1`
        );
  ...
  return (
    <div className="print-container">
      <div className="header">
        <h2 className="title">📦 Device 출력 현황</h2>
