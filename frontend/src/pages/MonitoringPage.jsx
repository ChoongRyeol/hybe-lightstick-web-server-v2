...
function MonitoringPage() {
  ...
  const SCROLL_KEY = "monitoring:scrollTop";
  ...
  const fetchStatus = useCallback(async () => {
    setIsLoading(true);
    setApiError("");
    console.log("[FETCH STATUS]", appliedFilters);
    try {
      const res = await fetch(
        `${SERVER_ADDRESS}/api/monitor/status-summary?from=${encodeURIComponent(appliedFilters.from)}&to=${encodeURIComponent(appliedFilters.to)}`,
        { credentials: "include" },
      );
  ...
  const goDetail = useCallback(
    (generatorName, extra = {}) => {
      ...
      navigate({
        pathname: `/monitor-detail/${encodeURIComponent(generatorName)}`,
        search: `?${qs.toString()}`,
      });
    },
  ...
  const handleDetailSearch = useCallback(() => {
    ...
    navigate({
      pathname: "/monitor/detail-advanced",
      search: `?${qs.toString()}`,
    });
  ...
  const fetchDailyProcess = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${SERVER_ADDRESS}/api/monitor/daily-process?${qs.toString()}`,
        { credentials: "include" },
      );
      const result = await res.json();
