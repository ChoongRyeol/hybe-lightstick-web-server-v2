...
function parseCommonQuery(req) {
  ...
  return {
    ...
    src: s(req.query.src),
    box_count: s(req.query.box_count),
  };
}

const PROC = {
  "firmware-download": {
    fromSql: "process_firmware_download_log t",
    dateCol: "created_at",
  ...
  "mac-write": {
    fromSql: "process_device_test_log t",
    dateCol: "created_at",
    orderCols: ["created_at", "id"],
    likeCols: [
      "serial",
  ...
  "device-print": {
    fromSql: "device_label_print_logs t",
    dateCol: "printed_at",
  ...
  "giftbox-print": {
    fromSql: "giftbox_label_print_logs t",
    dateCol: "printed_at",
    orderCols: ["printed_at", "id"],
    likeCols: [
      "generator_name",
...
function buildSelectSql(key) {
  if (key === "firmware-download") {
    return `
      SELECT
        t.*,
        HEX(t.device_guid) AS device_guid
    `;
  }
...
async function handleList(req, res, key) {
  ...
  const selectSql = buildSelectSql(key);
...
/** v2 라우트 */
router.get("/v2/firmware-download", (req, res) =>
  handleList(req, res, "firmware-download"),
);

router.get("/v2/firmware-download/locate", (req, res) =>
  handleLocate(req, res, "firmware-download"),
);
